...
export class TranslationProvider {
  ...
  blck_texts: any = {
    'en': (targs, nontargs, trefs, nontrefs, cittype) => {
      ...
      if (cittype !== 1) {
        thetexts.push(
          intro + intro_end + '<span id="feedback_id1"></span></br></br>In this first practice round, you have to categorize two kinds of items. ' + inducers_instructions +
          '<br><br>In each category, you need at least 80% correct responses in time.<br><br>');
        if (cittype === 0) {
          thetexts.push("<span id='feedback_id2'>In this second practice round, you have to categorize the main test items. " + main_instruction);
          thetexts.push(
          ...
          thetexts.push('In this second and last practice round, you also have to categorize the main test items: ' + nontargs + '. These all have to be categorized by touching the </i>left</i> button.');
        }
      } else {
        targs = '';
        thetexts.push(intro + main_instruction + intro_end);
        thetexts.push("<span id='feedback_id3'>Now, in this second and last practice round, you have to respond fast, but a certain rate of error is allowed. The task is the same.");
      }
