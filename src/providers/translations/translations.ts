...
export class TranslationProvider {
  ...
  blck_texts: any = {
    'en': (targs, nontargs, trefs, nontrefs, cittype) => {
      ...
      thetexts.push(
        "Now the actual test begins. The task is the same. Touch the <i>right</i> (→■) button when you see the following items:<br><div class = 'rightitems_list'>" + targs[0].replace('<br>', '') + trefs + "</div>Touch the <i>left</i> (■←) button for everything else.<br><br>Try to be both accurate and fast.");

      // repetitions (3 next probes)

      [1, 2, 3, 4, 5, 6, 7].forEach(pnum => {
        thetexts.push(
          'The main items will now be replaced with new ones. Touch the <i>right</i> (→■) button when you see the following target item:<br><div class = "rightitems_list">' +
          targs[pnum] +
          '</div>Touch the <i>left</i> (■←) button when you see any other item. These other items are:<br>' +
        ...
        thetexts.push(
          "Well done. The next block begins. The task is the same. Touch the <i>right</i> (→■) button when you see the following items:<br><div class = 'rightitems_list'>" + targs[pnum].replace('<br>', '') + trefs + "</div>Touch the <i>left</i> (■←) button for everything else.<br><br>Try to be both accurate and fast.");
      })
      return thetexts;
    },
    'hu': (targs, nontargs, trefs, nontrefs, cittype) => {
      ...
      thetexts.push(
        "<span id='feedback_id3'>A harmadik és utolsó gyakorlófeladat az összes elemet tartalmazza (" + ittypes + ").<br><br>Ismét gyorsan kell választ adnod, de egy bizonyos számú hiba megengedett. A feladat ugyanaz. Válaszd a <i>jobb oldali</i> gombot ha a következő elemek bármelyike jelenik meg:<br><div class = 'rightitems_list'>" + targs[0].replace('<br>', '') + trefs + "</div>Válaszd a <i>bal oldali</i> gombot ha bármely más elem jelenik meg.</span>");
      thetexts.push(
        "Most következik a teszt. A feladat ugyanaz. Válaszd a <i>jobb oldali</i> gombot ha az alábbi elemek bármelyike jelenik meg:<br><div class = 'rightitems_list'>" + targs[0].replace('<br>', '') + trefs + "</div>Válaszd a <i>bal oldali</i> gombot ha bármely más elem jelenik meg.<br><br>Probálj pontos és gyors lenni.");

      [1, 2, 3].forEach(pnum => {
        thetexts.push(
          'Most más fő elemek következnek. Válaszd a <i>jobb oldali</i> gombot ha az alábbi "cél" elem jelenik meg:<br><div class = "rightitems_list">' +
          targs[pnum] +
          '</div>Válaszd a <i>bal oldali</i> gombot ha bármely más elem jelenik meg. Ezek az elemek:<br>' +
        ...
        thetexts.push(
          "Rendben. A következő szakasz kezdődik. A feladat ugyanaz. Válaszd a <i>jobb oldali</i> gombot ha a következő elemek bármelyike jelenik meg:<br><div class = 'rightitems_list'>" + targs[pnum].replace('<br>', '') + trefs + "</div>Válaszd a <i>bal oldali</i> gombot ha bármely más elem jelenik meg.</span>");
      })
      return thetexts;
    }
  }
